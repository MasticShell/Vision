//! Size-reducing compression for image-heavy / large-format PDFs.
//!
//! M1 status: the `compress()` function has been stubbed out and returns
//! `CapabilityUnavailable`. The rasterisation step (pdftoppm JPEG) has been
//! removed. The `image_to_pdf()` path (wrapping an uploaded image file into a
//! one-page PDF) does NOT depend on Poppler and remains fully functional.
...
/// Compress `input` into `output` by rasterizing each page to JPEG.
///
...
pub fn compress(
    ...
    _job_id: &str,
    _picks: &[crate::models::PagePick],
    _output: &str,
    _dpi: u32,
    _quality: u32,
    _target_bytes: Option<u64>,
) -> Result<Vec<String>, AppError> {
    Err(AppError::new(
        "CapabilityUnavailable",
        "PDF compression temporarily unavailable",
        "PDF compression has been disabled during the M1 architecture migration. \
         The rasterisation pipeline that previously relied on Poppler (pdftoppm) \
